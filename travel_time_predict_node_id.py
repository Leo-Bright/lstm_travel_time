...
def extract_samples(travel_samples_file):
...
class lstm_reg(nn.Module):
    ...
    def forward(self, x):
        x, _ = self.rnn(x)  # (batch, seq, input_size) (seq, input_size) is the shape of inputs
        b, s, h = x.shape
...
for epo in range(epoch):
    ...
    samples_targets = extract_samples(samples_in_file_train)
    train_count = 0
